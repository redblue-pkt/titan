	...
	while(tmpcount--)
	{
		...
		if(filelist[i]->d_type == DT_DIR && filelist[i]->d_name != NULL && ostrcmp(filelist[i]->d_name, ".") != 0 && (status.showhiddenfiles == 1 || (status.showhiddenfiles == 0 && (filelist[i]->d_name[0] != '.' || ostrcmp(filelist[i]->d_name, "..") == 0))))
		{
			...
			if(child != NULL)
			{
				if(node->usesavebg == 3)
				{
					debug(555, "set child->usesavebg=%d to %d\n", child->usesavebg, node->usesavebg - 1);
					child->usesavebg = node->usesavebg - 1;
				}
				if(view == 2)
				{
					...
					if(child != parentdir) 
					{
						...
						else
						{
							...
							tmpstr = ostrcat(tmpstr, status.imdbfolderpath, 1, 0);
				...
				if(view > 3)
				{
					...
					else if(view == 5) child->filelist->date = getfiletime(rpath, 1);
					free(rpath); rpath = NULL;
					free(tmpstr); tmpstr = NULL;
				}
			}
