...
if [ "$CPU" = "mipsel" ];then
	drivername=`cat "$HOME"/flashimg/BUILDGIT/checkout_"$STM"/meta-oe-alliance/meta-brands/meta-ini/conf/machine/include/ini-oem.inc | grep "$BOXNAME'" | grep --regexp=[^mM]ini | cut -d "'" -f6`
