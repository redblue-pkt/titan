...
void checkhddaktiv()
{
	...
	while(status.hdd[count].device != NULL && count < 3)
	{
		tmpstr = ostrcat(tmpstr, "/sys/block/", 1, 0);
		tmpstr = ostrcat(tmpstr, status.hdd[count].device, 1, 0);
		tmpstr = ostrcat(tmpstr, "/stat", 1, 0);
		fd = fopen(tmpstr, "r");
		if(fd == NULL) {
			free(tmpstr);tmpstr=NULL;
			return;
		}
		fscanf(fd, "%d%d%d%d%d", &w1,&w2,&w3,&w4,&w5);
		fclose(fd);
		if((status.hdd[count].read != w1) || (status.hdd[count].write != w5))
		{
			status.hdd[count].read = w1;
			status.hdd[count].write = w5;
			status.hdd[count].notchanged = 0;
		} else {
			if (status.hdd[count].notchanged < status.hdd[count].sleeptime)
			{	
				status.hdd[count].notchanged++;
				if (status.hdd[count].notchanged == status.hdd[count].sleeptime)
				{
					free(tmpstr);tmpstr=NULL;
					tmpstr = ostrcat(tmpstr, "/sbin/sdparm -C stop /dev/", 1, 0);
					tmpstr = ostrcat(tmpstr, status.hdd[count].device, 1, 0);
					system(tmpstr);
				}
			}
...
void settimetosleep(int sekunden)
{
//	char zeitstr[5];
	char* tmpstr = NULL;
	char* disklist = NULL;
	...
	status.hdd[0].device=NULL;
	status.hdd[1].device=NULL;
	status.hdd[2].device=NULL;
	disklist = command("ls -1 -d /sys/block/sd* | sed 's!/sys/block/!!'");
	if(ostrcmp(string_newline(disklist), "") != 0){
		struct splitstr* ret1 = NULL;
		int count1 = 0;
		tmpstr = ostrcat("", disklist, 0, 0);
		ret1 = strsplit(tmpstr, "\n", &count1);
		max = count1;
		if(max > 3)
			max = 3; 
		for( i = 0; i < max; i++){
			status.hdd[i].device = ostrcat(status.hdd[i].device, (&ret1[i])->part, 1, 0);
			status.hdd[i].notchanged = 0;
			status.hdd[i].read = 0;
			status.hdd[i].write = 0;
			status.hdd[i].sleeptime= sekunden / 60;
		}
		free(ret1); ret1 = NULL;
		free(tmpstr);tmpstr=NULL;
	} else {
		free(disklist); disklist = NULL;
		return;
	}
	for( i = 0; i < max; i++){
		tmpstr = ostrcat(tmpstr, "hdparm -S", 1, 0);
		tmpstr = ostrcat(tmpstr, "0 /dev/", 1, 0);
		tmpstr = ostrcat(tmpstr, status.hdd[i].device, 1, 0);
		system(tmpstr);	
		free(tmpstr);tmpstr=NULL;
	}
	free(disklist); disklist = NULL;
	
	if(status.hddsleeptimer == NULL && sekunden > 0)
		status.hddsleeptimer = addtimer(&checkhddaktiv, START, 60000,-1, NULL, NULL, NULL);
	else if(sekunden <= 0 && status.hddsleeptimer != NULL)
		status.hddsleeptimer->aktion = STOP;
}
