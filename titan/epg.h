...
nextepg:
	...
	while(1)
	{
		...
		if(epgscreenconf == 0 && rcret == getrcconfigint("rcred", NULL))
		{
...
char* epgdescunzip(struct epg* epgnode)
{
	...
	if(epgnode->desccomplen < 1)
		return ostrcat(epgnode->desc, "", 0, 0);

	ret = unzip(epgnode->desc, epgnode->desccomplen, &zbuf, &zlen);
	if(ret == 0)
		return zbuf;
...
void parseeit(struct channel* chnode, unsigned char *buf, int len, int flag)
{
	...
	for (p = (unsigned char*)&eit->data; p < buf + len; p += EITEVENTLEN + GETEITDESCLEN(p))
	{
		...
		if(GETEITDESCLEN(evt) == 0 || chnode == NULL)
			return;
		...
		m_lock(&status.epgmutex, 4);
		epgnode = getepg(tmpchnode, HILO(evt->event_id), 1);
		if(epgnode != NULL)
		{
			...
			delepg(tmpchnode, HILO(evt->event_id), 1);
		}
		...
		parseeitdesc(epgnode, (unsigned char*)&evt->data, GETEITDESCLEN(evt));
		//compress long desc
		if(epgnode->desc != NULL)
		{
...
int readepg(const char* filename)
{
	...
	while(!feof(fd))
	{
		int serviceid = 0, transponderid = 0, eventid = 0;
		int version = 0, starttime = 0, endtime = 0;
...
read_more:
		...
		//del old epg date
		if(flag == 0 && status.deloldepg + (1 * 60 * 60) < time(NULL)) // 1 stunde
		{
