...
if [ "$buildtype" = "full" ];then
	...
	for ROUND in $list; do
		...
		for ROUND1 in $list1; do
			...
			picname=`echo $ROUND1 | sed 's/channel-logo /\nchannel-logo"/g' | grep ^"channel-logo" | cut -d'"' -f2`
