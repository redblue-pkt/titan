...
int smbInfo(char *pythonIp , char *pythonrName, char *pythonUser, char *pythonPass, shareinfo *sInfo) 
{
	...
	strupper(myname);
	sprintf(service1,"\\\\%s\\IPC$",pythonrName);
	strupper(service1);
#if DEBUG
	printf("service1 = %s\n", service1);
#endif
...
#endif
#if DEBUG
  if (strncmp(service1,"\\\\",2) != 0)
    {
      printf("\nWarning: Your service1 name doesn't start with \\\\. This is probably incorrect.\n");
      printf("Perhaps try replacing each \\ with \\\\ on the command line?\n\n");
    }
#endif

 again:
  set_message(outbuf,0,6 + strlen(service1) + strlen(pass) + strlen(dev),True);
