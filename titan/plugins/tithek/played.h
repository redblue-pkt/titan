...
char* played(char* link)
{
	...
	unlink("/tmp/played1_get");
	unlink("/tmp/played2_post");
	unlink("/tmp/played3_post");
	unlink("/tmp/played4_streamlink");
/////////////
...
*/
	...
	titheklog(debuglevel, "/tmp/played1_get", NULL, tmpstr);
	...
	free(send), send = NULL;
	debug(99, "tmpstr: %s", tmpstr);
	titheklog(debuglevel, "/tmp/played2_post", NULL, tmpstr);
	...
	titheklog(debuglevel, "/tmp/played4_streamlink", NULL, streamlink);
