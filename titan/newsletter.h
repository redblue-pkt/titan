...
int readnewsletter()
{
	...
	tmpstr = gethttp("TODO", "TODO", 80, newsletterfile, NULL, NULL, 0);
	...
	while(1)
	{
		...
		if(rcret == getrcconfigint("rcok", NULL))
		{
			if(listbox->select != NULL && listbox->select->handle != NULL)
			{
				...
				textbox(tmpstr, ((struct newsletter*)listbox->select->handle)->text, _("OK"), getrcconfigint("rcok", NULL), _("EXIT"), getrcconfigint("rcexit", NULL), NULL, 0, NULL, 0, 700, 600, 0, 2);
				free(tmpstr); tmpstr = NULL;
				drawscreen(skinnewsletter, 0, 0);
			}
		}
