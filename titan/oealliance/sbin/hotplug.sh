...
startRepairdev()
{
	echo "[hotplug.sh] startRepairdev: device=$1"
	#make a second mount check
	sleep 2
	ls -d "/autofs/$device" > /dev/null
	case $? in
		0) 
				echo "[hotplug.sh] startRepairdev: second mount for $device ok, no fschk";;
		*)
				killall -stop rcS
				killall -stop "$SWTYPE"			
...
startStickextensions() {
	echo "[hotplug.sh] startStickextensions: device=$device label=$label type=$type"
	
	if [ "$1" != "/var/swap" ] && [ "$1" != "/mnt/swapextensions" ]; then
		echo "[hotplug.sh] startStickextensions: parameter not ok ($1)"
		return
	fi
	
	if [ "$1" == "/var/swap" ]; then
		if [ ! -L "$1" ]; then
			echo "[hotplug.sh] startStickextensions: $1 is not a link device=$device label=$label type=$type"
			return
		fi
	fi
...
startCheckdev()
{
	#in dieser funkton k�nnen device checks aufgerufen werden
	
	echo "[hotplug.sh] startCheckdev: device=$device label=$label type=$type"
	...
	if [ ! -z $test ]; then
			...
			*)
				...
				case $? in 
					0)
						checkdev=0;;
					*)
						echo "[hotplug.sh] startCheckdev: can not mount device=$device label=$label type=$type"
		...
		echo "[hotplug.sh] startCheckdev: can not check ... automount is not running"
		checkdev=1
	fi
}
...
startIsreadonly()
{
	echo "[hotplug.sh] startIsreadonly: device=$device label=$label type=$type"
	...
	if [ $? -eq 0 ]; then
		...
		echo "[hotplug.sh] startIsreadonly: found read only device=$device label=$label type=$type"
		isreadonly=1
	fi
}
...
startChangedev()
{
	#in dieser funktion k�nnen deviceinfos wie z.B. label ge�ndert werden (wird bei jeder partition aufgerufen)
	
	echo "[hotplug.sh] startChangedev: device=$device label=$label type=$type"
}
...
startNewdev()
{
	...
	echo "[hotplug.sh] startNewdev: device=$device label=$label type=$type uuid=$uuid"
	...
	if [ -z "$swapdir" ] && [ "$label" == "SWAPPART-$device" ]; then
		echo "[hotplug.sh] startNewdev: found SWAPPART device=$device label=$label type=$type"
	...
	if [ "$type" != "vfat" ] && [ "$type" != "ext2" ] && [ "$type" != "ext3" ]  && [ "$type" != "ext4" ] && [ "$type" != "msdos" ] && [ "$type" != "jfs" ] && [ "$type" != "ntfs" ] && [ "$device" != "mtdblock5" ] && [ "$device" != "mtdblock4" ] && [ "$device" != "mtdblock6" ]; then
		echo "[hotplug.sh] startNewdev: found unsupportet medium device=$device label=$label type=$type"
		return
	fi
	...
	case $swapdir in
		"")
			if [ -d "/autofs/$device/swapdir" ]; then
				...
				case $isreadonly in
					0)
						echo "[hotplug.sh] startNewdev: found swapdir device=$device label=$label type=$type"
	...
	case $backup in
		"")
			if [ -d "/autofs/$device/backup" ]; then
				...
				case $isreadonly in
					0)
						echo "[hotplug.sh] startNewdev: found backup device=$device label=$label type=$type"
	...
	case $swapextensions in
		"")	
			if [ -d "/autofs/$device/swapextensions" ]; then
				...
				case $isreadonly in
					0)
						echo "[hotplug.sh] startNewdev: found swapextensions device=$device label=$label type=$type"
	...
	case $movie in
		"")
			if [ -d "/autofs/$device/movie" ]; then
				...
				case $isreadonly in
					0)
						echo "[hotplug.sh] startNewdev: found movie device=$device label=$label type=$type"
...
startCheckRecordShare() {
	echo "[hotplug.sh] startCheckRecordShare: start"
	if [ -e /mnt/network/.recordshare ]; then
		read recordshare < /mnt/network/.recordshare
		tmprecordshare=$recordshare
		
		echo "[hotplug.sh] startCheckRecordShare: check /autofs/$tmprecordshare/movie"
		if [ ! -z "$tmprecordshare" ] && [ ! -e "/autofs/$tmprecordshare/movie" ];then
			echo "[hotplug.sh] startCheckRecordShare: create /autofs/$tmprecordshare/movie"
			timeout -t 30 -s 9 mkdir "/autofs/$tmprecordshare/movie"
		fi
		if [ -e "/autofs/$tmprecordshare/movie" ];then
			echo "[hotplug.sh] startCheckRecordShare: found /autofs/$tmprecordshare/movie"
		fi
	fi
	...
	if [ ! -z "$recordshare" ] && [ ! -z "$fgr" ]; then
		...
		case $isreadonly in
		0)
			echo "[hotplug.sh] startCheckRecordShare: found NET movie device=$recordshare"
			echo "$recordshare" > /tmp/.moviedev
			movie=1
			rm -rf /media/hdd
			echo "[hotplug.sh] startCheckRecordShare: linking /media/hdd > /autofs/$recordshare"
	...
	for share in $sharelist; do
		...
		echo "[hotplug.sh] startCheckRecordShare: linking /media/net/$share > /autofs/$share"
		rm -rf "/media/net/$share"
		ln -sn "/autofs/$share" "/media/net/$share"
	done
	echo "[hotplug.sh] startCheckRecordShare: ende"
}

startCheckhanging()
{
	echo "[hotplug.sh] startCheckhanging: start"
	
	sleep 60

	kill -9 "$1" 2>&1 >/dev/null
	if [ $? -eq 0 ]; then
		echo "[hotplug.sh] startCheckhanging: timeout"
		> /tmp/.hotplugready
	fi
	echo "startCheckhanging: ende"
}
...
case $startmode in
	first)	
		...
		naslist=`awk '/(nfs)|(cifs)/ { print substr($3,15) }' /proc/mounts`
...
# 				edit obi add nasshars this is better as recordshare, we can used backup/swapdir on nas
					case $backup in
						"")
							if [ -e "/autofs/$nasshare/backup" ]; then
								echo "[hotplug.sh] first: found NET backup device=$nasshare"
					...
					if [ -e "/autofs/$nasshare/swapextensions" ]; then
						echo "[hotplug.sh] first: found NET swapextensions device=$nasshare"
					...
					case $swapdir in
						"")
							if [ -e "/autofs/$nasshare/swapdir" ]; then
								echo "[hotplug.sh] first: found NET swapdir device=$nasshare"
...
case $ACTION in
	add)
		...
		echo "[hotplug.sh] add: ACTION=add device=$device"
	...
	remove)
		...
		startWaitfor "/tmp/.hotplugready" 1000; rm -f /tmp/.hotplugready
		echo "[hotplug.sh] remove: ACTION=remove device=$device"
