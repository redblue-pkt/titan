...
#        print "xxxxxxxx5555"
#        common.log_utils.log(sources)
        sources = [source for source in sources if not any([x in source[1].lower() for x in ('/movie.mp4', '/trailer.mp4', '://cdn.flashx.tv')])]
#        print "xxxxxxxx6666"
#        print "source", source
...
def adblock_check(js, headers):
    ...
    if match:
        ...
        fx_url = resolve_url(urlparse.urljoin('https://www.flashx.tv', fx_url) + '?' + urllib.urlencode({fx_param: fx_value}))
...
#        common.log_utils.log(res_headers)
        if res_headers.get('Etag', '').strip('"') not in BAD_ETAGS and int(res_headers.get('Content-Length', 0)) >= MIN_CONTENT:
            return True
    except Exception as e:
