...
char* gettunerlockpic(struct skin* node, char* path, char* tuner)
{
	...
	if(tunerlock > 0 || (status.aktservice->fedev != NULL && status.aktservice->fedev->devnr == tunernr && status.standby == 0 && status.playing == 0))
	{
		if(node->nodestyle != 1)
			node->hidden = NO;	
		if(node->picmem == 1)
...
char* gettunerlocktext(struct skin* node, char* onlyhidden, char* tuner)
{
	...
	if(tunerlock > 0 || (tunerlock == 0 && status.aktservice->fedev != NULL && status.aktservice->fedev->devnr == tunernr && status.standby == 0 && status.playing == 0))
	{
		if(node->nodestyle != 1)
			node->hidden = NO;
		if(onlyhidden == NULL)
