...
// nit: dekativate cache, does not work with all dvd
// on some dvd menu is not allays displayed
//#if defined(__sh__)
...
//needed for titan
#if defined(__sh__)
					playerconfig->dvd_aspect = dvd_aspect;
#endif
...
// nit: VIDEO_GET_EVENT blocks here
// if dvd can't start VIDEO_GET_PTS overflows console with output
#if defined(__sh__)
