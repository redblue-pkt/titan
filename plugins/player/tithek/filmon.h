...
int filmon_search_local(struct skin* grid, struct skin* listbox, struct skin* countlabel, struct skin* load, char* link, char* title, char* searchstr, int flag)
{
	...
	if(search != NULL)
	{
		...
		tmpstr = gethttp("openaaf.dyndns.tv", "/mediathek/filmon/streams/filmon.all-sorted.list", 80, NULL, HTTPAUTH, 5000, NULL, 0);
		...
		if(ret1 != NULL)
		{
			...
			if(line != NULL)
			{
				...
				menu = ostrcat("/tmp/tithek/filmon.search.list", NULL, 0, 0);
