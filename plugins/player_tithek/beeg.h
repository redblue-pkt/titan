...
char* beeg(char* link)
{
	...
	titheklog(debuglevel, "/tmp/beeg1_tmpstr", NULL, NULL, NULL, tmpstr);
	...
	titheklog(debuglevel, "/tmp/beeg2_streamurl", NULL, NULL, NULL, streamurl);
...
int beeg_search_local(struct skin* grid, struct skin* listbox, struct skin* countlabel, struct skin* load, char* link, char* title, char* searchstr, int flag)
{
	...
	if(search != NULL)
	{
		...
		tmpstr = gethttp("openaaf.dyndns.tv", "/mediathek/beeg/streams/beeg.all-sorted.list", 80, NULL, HTTPAUTH, 5000, NULL, 0);
