...
int python_hostinfo(struct in_addr addr, const struct nb_host_info* hostinfo, netinfo *nInfo, int pos)
{
  ...
  my_uint8_t service;
	...
	tmpstr = (char*)getnbservicename(service, unique, hostinfo->names[0].ascii_name);
	sprintf(nInfo[pos].service,"%s", tmpstr);
...
int netzInfo(char *pythonIp, netinfo *nInfo) {
  ...
  if(!buff)
  {
	free(fdsw); fdsw = NULL;
	free(fdsr); fdsr = NULL;
  	free(next_in_addr); next_in_addr = NULL;
	err_die("Malloc failed", quiet);
  }
  ...
  for(i=0; i <= retransmits; i++) {
	...
	if(insert(scanned, ntohl(dest_sockaddr.sin_addr.s_addr))) {
	  ...
	  rttvar += (delta - rttvar) / 4 ;
        ...
        free(hostinfo->header); hostinfo->header = NULL;
        free(hostinfo->names); hostinfo->names = NULL;
        free(hostinfo->footer); hostinfo->footer = NULL;
	free(hostinfo); hostinfo = NULL;
  };
  ...
  delete_list(scanned);
  if(buff) free(buff);
  if(next_in_addr) free(next_in_addr);
  if(fdsr) free(fdsr);
  if(fdsw) free(fdsw);
  return 0;
};
