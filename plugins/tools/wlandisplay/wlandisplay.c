...
void LCD_Samsung1_thread()
{
	...
	while (LCD_Samsung1thread->aktion != STOP) {
		...
		if(status.standby > 0 && standby == 0)
		{
			...
			{
				tmpstr = createpluginpath("/wlandisplay/standby.jpg", 0);
...
void LCD_Samsung1_main()
{
	if(LCD_Samsung1thread == NULL)
	{
		...
		tmpstr = createpluginpath("/wlandisplay/start.png", 0);
		tmpstr = ostrcat("cp ", tmpstr, 0, 1);
		tmpstr = ostrcat(tmpstr, " /tmp/titanlcd.png", 1, 0);
		system(tmpstr);
		free(tmpstr); tmpstr=NULL;
...
void init(void)
{
	...
	tmpstr = ostrcat("/var/usr/local/share/titan/plugins/tools/wlandisplay/skin2.xml", NULL, 0, 0);
	if(!file_exist(tmpstr))
	{
		free(tmpstr); tmpstr = NULL;
		tmpstr = ostrcat("/var/swap/usr/local/share/titan/plugins/tools/wlandisplay/skin2.xml", NULL, 0, 0);
	}
	if(!file_exist(tmpstr))
	{
		free(tmpstr); tmpstr = NULL;
		tmpstr = ostrcat("/mnt/swapextensions/usr/local/share/titan/plugins/tools/wlandisplay/skin2.xml", NULL, 0, 0);
	}
	if(!file_exist(tmpstr))
	{
		free(tmpstr); tmpstr = NULL;
		tmpstr = createpluginpath("/wlandisplay/skin.xml", 0);
	}
