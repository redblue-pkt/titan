...
char* videoweed(char* link)
{
	...
	unlink("/tmp/videoweed1_get");
	unlink("/tmp/videoweed2_get");
/////////////
...
*/
	...
	titheklog(debuglevel, "/tmp/videoweed1_get", NULL, NULL, NULL, tmpstr);
	...
	tmppath = ostrcat("/api/player.api.php?user=undefined&codes=1&file=", file, 0, 0);
	tmppath = ostrcat(tmppath, "&pass=undefined&key=", 1, 0);
	...
	tmpstr = gethttpreal(tmphost, tmppath, 80, NULL, NULL, NULL, 0, send, NULL, 5000, 1);
	debug(99, "tmpstr: %s", tmpstr);
	titheklog(debuglevel, "/tmp/videoweed2_get", NULL, NULL, NULL, tmpstr);
