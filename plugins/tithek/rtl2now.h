...
char* rtl2now(char* link, char* url, char* name, int flag)
{
	...
	if(flag == 1)
	{
		if(ostrstr(tmpstr, "<!-- 1-->") != NULL)
			textbox(_("Message"), _("Found Pay Stream visit the officle Website and Order this Stream and try this again !") , _("OK"), getrcconfigint("rcok", NULL), _("EXIT"), getrcconfigint("rcexit", NULL), NULL, 0, NULL, 0, 1200, 400, 0, 0);
