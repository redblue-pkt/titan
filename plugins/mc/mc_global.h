...
void info_menu(struct skin* apskin, struct skin* filelist, struct skin* filelistpath, char* filename, int* playinfobarstatus, int* playinfobarcount, int playertype, int flag, int files)
{
	...
	if(status.play == 1)
	{
		addmenulist(&mlist, "Infobar", NULL, NULL, 0, 0);
	}
	...
	if(mbox != NULL)
	{
		...
		else if(ostrcmp(mbox->name, "Infobar") == 0)
...
//			get_file_info(apskin, filelist, filelistpath);
	}
...
/*
void get_file_info(struct skin* apskin, struct skin* filelist, struct skin* filelistpath)
{
	char* tmpstr = NULL;
	char* pic = NULL;
	char* cmd = NULL;
